...
This artifact is associated with the full paper article accepted in the Main Track of the ACSOS 2025 conference.
...
- 4GB free space on disk (the system will automatically download its dependencies through Gradle) (**NOTE** that if you want to download or generate data as in [Reproduce the experiment results](#reproduce-the-experiment-results), you will need an additional ~15GB free space);
...
│   │   │   ├── model           # Alchemist's model for the experiments such as random failures, move strategies, location sensors, etc.
│   │   ├── collektive.program  # The main Collektive logic for the experiments
│   │   │   ├── Follower.kt     # The entrypoint for the baseline experiments
│   │   │   ├── Replanning.kt   # The entrypoint for the runtime replanning experiments
│   │   ├── formalization       # Formalization of the experiments, such as allocation strategies and task completion
│   ├── resources   # Resources (such as images) for the experiments environment
...
3. Discover available robots: Gossip node coordinates to build a global view of all robots in the system within communication range
4. Check stability condition: Evaluate whether the current system state is stable enough to continue with existing plans
5. Replan if unstable: When stability condition fails, filter out completed tasks and run a greedy allocation strategy to generate new global plans for all robots
6. Execute plan: Either follow the newly computed plan (if replanning occurred) or continue with the existing plan (if system is stable)
...
2. Gather network information: Collect all node IDs in the system and gossip node coordinates to build a view of available robots
3. Check stability conditions: Verify if the current leader is stable over a time window and if the robot network composition is stable
4. Leader-driven replanning: Only the leader triggers replanning when either the leader role is unstable or the robot network changes, using a greedy allocation strategy on uncompleted tasks
5. Distribute plans via gradient: Use gradient cast to propagate the new plan from the leader to all nodes in the network following the distance field
6. Execute coordinated action: All nodes follow their assigned plans when a stable leader exists, otherwise continue with previous allocations
...
2. If you have downloaded the data from the provided link, make sure to move the extracted `data` folder inside the repository.
